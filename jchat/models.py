...
class Room(models.Model):
    ...
    def __add_message(self, type, sender, user=None,message=None):
        '''Generic function for adding a message to the chat room'''
        m = Message(room=self, type=type, author=sender, message=message, user=user)
        m.save()
        return m
    ...
    def say(self, sender, user, message):
        ...
        return self.__add_message('m', sender, user,message)
...
class Message(models.Model):
    ...
    user = models.ForeignKey(User, related_name='author', blank=True, null=True)
    ...
    def __unicode__(self):
        ...
        if self.type == 'n':
            ...
            return '%s JOIN: %s' % (self.timestamp.strftime('%d.%m.%Y %H:%M:%S'),self.author)
        elif self.type == 'l':
            return '%s LEAVE: %s' % (self.timestamp.strftime('%d.%m.%Y %H:%M:%S'),self.author)
        elif self.type == 'a':
            return '%s ACTION: %s > %s' % (self.timestamp.strftime('%d.%m.%Y %H:%M:%S'),self.author, self.message)
        elif self.type == 'c':
            return '%s Command: %s > %s' % (self.timestamp.strftime('%d.%m.%Y %H:%M:%S'),self.author, self.message)
        return self.message
    def parse_tags(self):
        if self and self.message and self.type == 'm':
            return [w for w in self.message.split() if w.startswith('#')]
        return []
