...
def project_json(request, project_id):
    ...
    if project:
        ...
        actions = list(Fileaction.objects.filter(Q(file__project=project) | Q(session__project=project)).exclude(Q(file__path__icontains='Snapshots') | Q(file__path__icontains='Audio') | Q(file__path__icontains='Screenshots') | Q(file__path__icontains='~$')).order_by('action_time').values('id', 'file__path', 'file__id', 'action__name', 'action_time'))
